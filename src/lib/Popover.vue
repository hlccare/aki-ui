...
export default {
  ...
  setup(props) {
    ...
    onUnmounted(() => {
      if (trigger === "click") {
        popover.value && popover.value.removeEventListener("click", onClick);
      } else {
        popover.value && popover.value.removeEventListener("mouseenter", open);
        popover.value && popover.value.removeEventListener("mouseleave", close);
      }
    });
    const locateContent = () => {
      let {
    ...
    const onClickDocument = (event) => {
      if (
        triggerWrapper.value.contains(event.target) ||
        (contentWrapper.value && contentWrapper.value.contains(event.target))
      ) {
        return;
      } else {
        close();
